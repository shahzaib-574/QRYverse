...
- The AdMob Android banner-unit form remains staged at its final **Create** action. No production ad unit was created.
...
1. Review the checkpoint diff and rerun `npm run lint`, `npm test`, `python scripts/prepare-play-screenshots.py`, and `npm run android:local-verify` if source or dependencies have changed.
...
The goal is intentionally paused, not marked complete: a signed production AAB, final legal/public URLs, production advertising configuration, signed-device screenshots, Play Console evidence, and hardware testing remain required.
