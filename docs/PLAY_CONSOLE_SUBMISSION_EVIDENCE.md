...
| AdMob ownership/readiness | `public/app-ads.txt.example` is only a template. No production developer domain, production publisher ID, verified app-ads.txt status, store link, or Ready status is proven. | **External blocker** | Publish the personalized `app-ads.txt` at the root of the developer-website host, link that website from Play, verify HTTP 200/crawlability and publisher ID, link QRYverse to its Play listing in AdMob, request/complete app verification and app-readiness review, and record Ready status. |
