...
| New personal account closed-test gate | The developer account type and creation date are unknown. For personal accounts created after November 13, 2023, Google requires at least 12 testers continuously opted into a closed test for the preceding 14 days, followed by a production-access application. | **External blocker if applicable** | Record account type/creation date. If applicable, preserve the closed-track name, opt-in URL, 12+ continuously opted-in tester evidence, start/end timestamps, feedback summary, changes made, and production-access approval. |
| New personal account device verification | Account applicability and verification status are unknown. New personal accounts can be required to verify access to a non-rooted physical Android device running Android 10 or later using the Play Console mobile app. | **External blocker if applicable** | Account-owner screenshot/export showing the verification task completed. |
| Developer identity and package registration | No Play Console evidence is available. Google says all Play package names must be registered by September 30, 2026; QRYverse's package is `com.royal.qrystudio`. | **External blocker; time-sensitive** | Verify developer identity and confirm `com.royal.qrystudio` is registered/auto-registered in Play Console before September 30, 2026. Save the status and applicable certificate details. |
| AdMob ownership/readiness | Read-only Console verification on August 24, 2026 showed an approved publisher account with no current Policy center issues and an existing QRYverse Android app. QRYverse itself has no ad unit or store details and remains **Requires review**. No European/US privacy message exists; the inherited maximum ad-content rating is MA while the documented G override is only staged. `public/app-ads.txt.example` remains a template. | **External blocker** | With explicit owner confirmation, create the staged banner and save the G override; create and publish the applicable privacy messages. Publish personalized `app-ads.txt` at the developer-website root, link the Play listing, verify HTTP 200/crawlability and publisher ID, request/complete app review, and record Ready status. |
