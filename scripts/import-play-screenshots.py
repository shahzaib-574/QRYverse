"""Import the eight canonical browser captures as real Google Play PNG assets."""
...
DESTINATION = ROOT / "store-assets" / "screenshots"
STEMS = (
    "01-home",
    "02-scan-result",
    "03-create",
...
def main() -> None:
    ...
    parser.add_argument("source", type=Path, help="Directory containing the eight canonical .jpg browser captures")
    args = parser.parse_args()
    source = args.source.resolve()
    if not source.is_dir():
        raise SystemExit(f"Capture directory does not exist: {source}")

    sources = tuple(source / f"{stem}.jpg" for stem in STEMS)
    missing = tuple(path.name for path in sources if not path.is_file())
    if missing:
        raise SystemExit(f"Missing canonical captures: {missing}")
    ...
    for input_path, stem in zip(sources, STEMS, strict=True):
        with Image.open(input_path) as captured:
            if captured.format != "JPEG":
                raise SystemExit(f"{input_path.name}: expected a JPEG browser capture, got {captured.format}")
    ...
    try:
        ...
        for input_path, (target, image), temporary in zip(sources, prepared, temporary_paths, strict=True):
            ...
            print(f"{input_path.name} -> {target.relative_to(ROOT)}  {image.width}x{image.height}  RGB PNG")
    except Exception:
