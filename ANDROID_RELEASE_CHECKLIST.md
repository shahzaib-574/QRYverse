...
11. If the Play developer account is a new personal account, complete Google's [real Android device verification](https://support.google.com/googleplay/android-developer/answer/14316361?hl=en) in the Play Console mobile app using an eligible, non-rooted physical device running Android 10 or later before making the app available.
12. If the Play developer account is a personal account created after November 13, 2023, complete Google's mandatory [production-access test](https://support.google.com/googleplay/android-developer/answer/14151465?hl=en): keep at least 12 testers continuously opted in to a closed test for at least 14 days, then apply for production access. Internal testing does not satisfy this gate.
13. Confirm backup/restore, local-data deletion and clearing, crash handling, TalkBack, 200% font scaling, reduced motion, offline behavior, camera permission paths, and rotation on physical devices. On a clean install, verify Play pre-delivers the declared ML Kit `barcode_ui` module and that first-use live scanning works after the app is taken offline; the app must still surface its retry message if the device's Google scanner module is unavailable.
